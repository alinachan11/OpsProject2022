module "vpc_module" {
  source                    = "app.terraform.io/alina-ops/my-vpc-module/alinaops"
  version = "1.0.2-alpha"
...
module "jenkins_module" {
  ...
  security_groups = [aws_security_group.ssh-sg.id,aws_security_group.common-sg.id,aws_security_group.jenkins-accesss-sg.id,aws_security_group.https-sg.id]
...
module "sd_module" {
  ...
  security_groups = [aws_security_group.ssh-sg.id,aws_security_group.common-sg.id]
}
...
module "EKS_Module" {
  ...
  more_sg = [aws_security_group.common-sg.id,aws_security_group.elk-sg.id,aws_security_group.prom-sg.id]
}
